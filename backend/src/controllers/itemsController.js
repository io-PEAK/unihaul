...
async function getItemTransactionState(itemId) {
  const [pendingTxn, completedTxn] = await Promise.all([
  ...
  return {
    hasPendingTxn: Boolean(pendingTxn),
    hasCompletedTxn: Boolean(completedTxn),
  };
}
  ...
  try {
    ...
    if (institutions) {
      ...
      if (list.length > 0)
        locationFilters.push({
          sellerInstitution: { in: list, mode: "insensitive" },
        });
    }
    ...
    if (cities) {
      try {
        ...
        const cityConditions = Object.entries(cityMap).map(
          ([state, cityList]) => ({
            AND: [
    ...
    if (sortPrice === "asc")
      return res.json(items.slice().sort((a, b) => a.price - b.price));
    if (sortPrice === "desc")
      return res.json(items.slice().sort((a, b) => b.price - a.price));
...
export const createItem = async (req, res) => {
  ...
  if (parsedPrice > MAX_ITEM_PRICE)
    return res.status(400).json({
      error: `Price cannot exceed ₹${MAX_ITEM_PRICE.toLocaleString("en-IN")}.`,
    });
...
export const updateItem = async (req, res) => {
  ...
  try {
    ...
    if (!item) return res.status(404).json({ error: "Item not found." });
    if (item.sellerId !== parseInt(userId))
    ...
    if (status !== undefined && status !== item.status) {
      if (status === "pending") {
        return res.status(400).json({
          error:
            "Manual pending status is disabled. Item moves to sold only after successful payment confirmation.",
        });
      }

      const { hasPendingTxn, hasCompletedTxn } =
        await getItemTransactionState(id);

      if (hasPendingTxn) {
        return res.status(409).json({
          error:
            "This item has an active checkout. Status cannot be changed right now.",
        });
      }

      if (hasCompletedTxn && status !== "sold") {
        return res.status(409).json({
          error:
            "This item already has a completed transaction and cannot be set back to available.",
        });
      }
    }
    ...
    if (updated.status?.toLowerCase() === "sold") {
      await prisma.watchedItem.deleteMany({ where: { itemId: parseInt(id) } });
    }
    ...
    if (updated.status?.toLowerCase() !== "sold" && parsedPrice < item.price) {
      ...
      for (const watcher of watchers) {
        if (!watcher.user.priceDropAlerts || !watcher.user.notificationsEnabled)
          continue;
...
export const updateItemStatus = async (req, res) => {
  ...
  try {
    ...
    if (hasPendingTxn) {
      return res.status(409).json({
        error: "This item has an active checkout. Status cannot be changed.",
      });
    }
    ...
    const [updated] = await prisma.$transaction([
      ...
      ...(status === "sold"
        ? [prisma.watchedItem.deleteMany({ where: { itemId: parseInt(id) } })]
        : []),
    ]);
    res.json(updated);
  } catch (err) {
...
export const deleteItem = async (req, res) => {
  ...
  try {
    ...
    const allUrls = [
      ...
      ...(item.imageUrl && !item.images?.includes(item.imageUrl)
        ? [item.imageUrl]
        : []),
    ].filter(Boolean);
...
export const watchItem = async (req, res) => {
  ...
  try {
    ...
    if (!item) return res.status(404).json({ error: "Item not found." });
    if (item.sellerId === parseInt(userId))
      return res.status(400).json({ error: "Cannot watch your own item." });

    const watch = await prisma.watchedItem.upsert({
      where: {
        userId_itemId: { userId: parseInt(userId), itemId: parseInt(id) },
      },
      update: {},
      create: {
        userId: parseInt(userId),
        itemId: parseInt(id),
        priceAtWatch: item.price,
      },
    });
...
export const getWatchedItems = async (req, res) => {
  const userId = req.user.userId;
  try {
    // Keep watchlist clean by pruning sold items for this user.
    await prisma.watchedItem.deleteMany({
      where: {
        userId: parseInt(userId),
        item: { status: "sold" },
