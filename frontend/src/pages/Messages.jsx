...
function Avatar({ name, size = 36, orange = false, src = null }) {
  ...
  if (src && !imgFailed) return (
    <div style={{
      ...
      <img src={src} alt={name} referrerPolicy="no-referrer" onError={() => setImgFailed(true)}
...
function ConversationItem({ convo, isActive, isSelected, selectMode, onClick, onSelect, isTyping }) {
  ...
  return (
    ...
    >
      ...
      {selectMode && (
        <div style={{ width: '20px', height: '20px', borderRadius: '6px', border: isSelected ? 'none' : '1.5px solid var(--border-hover)', background: isSelected ? 'var(--accent)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.15s' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: '0.88rem', fontWeight: hasUnread ? '800' : '600', color: isActive ? 'var(--accent)' : hasUnread ? 'var(--text-primary)' : 'var(--text-secondary)', letterSpacing: '-0.2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '130px' }}>
            {convo.other_user_name || 'Unknown'}
          </span>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            {/* Context Icon (Profile vs Item) */}
            <div style={{ color: isActive ? 'var(--accent)' : 'var(--text-ghost)', display: 'flex', alignItems: 'center', opacity: isActive ? 1 : 0.6 }}>
              {isProfileChat ? (
                ...
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>
              )}
            </div>
            {hasUnread && !isActive && !selectMode && (
        ...
        {isTyping ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '3px', marginTop: '0.3rem' }}>
            {[0, 1, 2].map(i => (
              <div key={i} style={{ width: '5px', height: '5px', borderRadius: '50%', background: '#a8c4a2', animation: 'typingDot 1.1s ease-in-out infinite', animationDelay: `${i * 0.18}s` }} />
            ))}
          </div>
        ) : convo.chat_request_status === 'pending' && convo.is_request_sender ? (
          <div style={{ fontSize: '0.68rem', color: 'var(--accent)', fontWeight: '600', marginTop: '0.2rem', display: 'flex', alignItems: 'center', gap: '0.3rem', opacity: 0.85 }}>
          ...
          <div style={{ fontSize: '0.72rem', color: hasUnread && !isActive ? 'var(--text-secondary)' : 'var(--text-ghost)', fontWeight: hasUnread && !isActive ? '600' : '400', marginTop: '0.2rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {convo.last_message}
          </div>
        )}
      </div>
...
function ItemCard({ convo, myId, onStatusChange }) {
  ...
  useEffect(() => { setItemImage(convo.item_image || null) }, [convo.item_id])
            ...
            <div style={{ marginTop: '0.15rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              {editingPrice ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                  ...
                  <input ref={priceInputRef} type="number" min="0" value={priceVal}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
          {isSeller && !isSold ? (
            <button onClick={toggleStatus} disabled={loading}
              onMouseEnter={() => setStatusHovered(true)} onMouseLeave={() => setStatusHovered(false)}
...
function Messages() {
  ...
  useEffect(() => {
    ...
    socket.on('online-list', ({ userIds }) => {
      if (activeConvo?.other_user_id) setOtherUserOnline(userIds.includes(String(activeConvo.other_user_id)))
    })
    ...
    socket.on('user-online', ({ userId }) => {
      if (activeConvo && String(userId) === String(activeConvo.other_user_id)) setOtherUserOnline(true)
    })
    socket.on('user-offline', ({ userId }) => {
      if (activeConvo && String(userId) === String(activeConvo.other_user_id)) setOtherUserOnline(false)
    })
    ...
    socket.on('request-accepted', ({ requestId, itemId, seller }) => {
      ...
      API.get('/messages/conversations').then(res => {
        ...
        const accepted = res.data.find(c =>
          c.conversation_id === itemKey && c.chat_request_status === 'accepted'
        )
        if (accepted) { setActiveConvo(accepted) }
      }).catch(() => {})
    })
    socket.on('request-declined', ({ requestId }) => {
      setChatRequests(prev => prev.filter(r => r.id !== requestId))
      setConversations(prev => prev.filter(c => c.chat_request_id !== requestId))
      if (activeConvo?.chat_request_id === requestId) { setActiveConvo(null); setMessages([]) }
    })
    ...
    socket.on('new-message', (msg) => {
      const isActiveConvo = activeConvo &&
        msg.itemId === activeConvo.item_id &&
        (String(msg.senderId) === String(activeConvo.other_user_id) || String(msg.senderId) === String(myId))
      ...
      if (isActiveConvo) {
        ...
        setConversations(prev => prev.map(c =>
          String(c.other_user_id) === String(activeConvo.other_user_id) && c.item_id === activeConvo.item_id
            ? { ...c, unread_count: 0, last_message: msg.content, last_message_at: msg.createdAt }
            : c
        ))
        if (activeConvo.isNew) {
          API.get('/messages/conversations').then(res => {
            setConvosDedupe(res.data)
            const real = res.data.find(c =>
              c.conversation_id === activeConvo.conversation_id && !c.isNew
            ) || res.data.find(c =>
              String(c.other_user_id) === String(activeConvo.other_user_id) &&
              String(c.item_id ?? 'null') === String(activeConvo.item_id ?? 'null') && !c.isNew
            )
          ...
          setConversations(prev => {
            const updated = prev.map(c =>
              c.conversation_id === activeConvo.conversation_id
                ? { ...c, last_message: msg.content, last_message_at: msg.createdAt }
                : c
            )
            const idx = updated.findIndex(c => c.conversation_id === activeConvo.conversation_id)
            if (idx > 0) { const [m] = updated.splice(idx, 1); return [m, ...updated] }
            return updated
          })
        }
        if (String(msg.senderId) === String(activeConvo.other_user_id)) {
          API.post('/messages/mark-convo-read', { itemId: msg.itemId, otherUserId: msg.senderId }).catch(() => {})
        }
      } else if (String(msg.receiverId) === String(myId)) {
        setConversations(prev => {
          const updated = prev.map(c =>
            String(c.other_user_id) === String(msg.senderId) && c.item_id === (msg.itemId ?? null)
              ? { ...c, unread_count: (c.unread_count || 0) + 1, last_message: msg.content, last_message_at: msg.createdAt }
              : c
          )
    ...
    return () => {
      socket.off('online-list'); socket.off('user-online'); socket.off('user-offline')
      socket.off('typing-start'); socket.off('typing-stop')
      socket.off('request-accepted'); socket.off('request-declined'); socket.off('new-chat-request'); socket.off('new-message')
      clearTimeout(typingClearTimeoutRef.current); clearTimeout(typingEmitTimeoutRef.current)
    }
  ...
  useEffect(() => {
    ...
    API.get('/chat-requests').then(res => {
      setChatRequests(res.data?.received?.filter(r => r.status === 'pending') || [])
    }).catch(() => {})
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        ...
        const res = await API.get(`/messages/${itemIdParam}`, { params: { otherUserId: activeConvo.other_user_id } })
        ...
        await API.post('/messages/mark-convo-read', { itemId: activeConvo.item_id, otherUserId: activeConvo.other_user_id }).catch(() => {})
  ...
  async function handleRespondRequest(id, status) {
    ...
    try {
      ...
      if (status === 'accepted') {
        ...
        const accepted = convosRes.data.find(c =>
          c.conversation_id === itemKey && c.chat_request_status === 'accepted'
        )
        if (accepted) { setActiveConvo(accepted) }
      }
    } catch (err) {
      console.error('Failed to respond to request:', err?.response?.data || err)
  ...
  async function handleDeleteConfirm() {
    ...
    try {
      ...
      await API.delete(`/messages/conversation/${itemIdParam}/${confirmTarget.other_user_id}`)
      setConversations(prev => prev.filter(c => c.conversation_id !== confirmTarget.conversation_id))
      if (activeConvo?.conversation_id === confirmTarget.conversation_id) { setActiveConvo(null); setMessages([]) }
  ...
  async function handleDeleteSelected() {
    ...
    try {
      ...
      await Promise.allSettled(toDelete.map(c => API.delete(`/messages/conversation/${c.item_id ?? 'null'}/${c.other_user_id}`)))
  ...
  async function handleSend(e) {
    ...
    try {
      ...
      const res = await API.post('/messages', { receiverId: activeConvo.other_user_id, itemId, content })

      if (res.data?.routedAsRequest) {
        const convosRes = await API.get('/messages/conversations').catch(() => null)
        if (convosRes?.data) {
          setConvosDedupe(convosRes.data)
          const pending = convosRes.data.find(c =>
            String(c.other_user_id) === String(activeConvo.other_user_id) &&
      ...
      if (activeConvo.isNew) {
        const convosRes = await API.get('/messages/conversations')
        setConvosDedupe(convosRes.data)
        const real = convosRes.data.find(c =>
          c.conversation_id === activeConvo.conversation_id && !c.isNew
      ...
      if (socket && activeConvo?.other_user_id) {
        ...
        socket.emit('typing-stop', { toUserId: activeConvo.other_user_id, itemId })
      }
    } catch (err) {
      ...
      <div style={{ maxWidth: '1200px', width: '100%', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.5rem', flex: 1, minHeight: 0 }}>
        ...
        {mobShowChat && (
          <div className="msgs-drawer" onClick={() => setMobShowChat(false)}>
            <div style={{ position:'absolute', inset:0, background:'rgba(0,0,0,0.6)', backdropFilter:'blur(4px)', borderRadius:'20px' }} />
            <div onClick={e => e.stopPropagation()}
              style={{ position:'absolute', top:'0', left:'0', bottom:'0', width:'88%', maxWidth:'310px', background:'var(--bg-surface)', border:'1px solid var(--border)', borderRadius:'20px 0 0 20px', display:'flex', flexDirection:'column', overflow:'hidden', boxShadow:'8px 0 40px rgba(0,0,0,0.5)', zIndex:51, animation:'drawerIn 0.22s cubic-bezier(0.22,1,0.36,1)' }}>

              {/* Tab headers */}
              <div style={{ borderBottom:'1px solid var(--border)', flexShrink:0 }}>
                <div style={{ display:'flex' }}>
                  {[{ key:'chats', label:'Chats' }, ...(chatRequests.length > 0 ? [{ key:'requests', label:'Requests', count: chatRequests.length }] : [])].map(tab => (
                    <button key={tab.key} onClick={() => { setSidebarTab(tab.key); if (tab.key === 'chats') setActiveRequest(null) }}
                      style={{ flex:1, padding:'0.75rem 0.5rem', background:'none', border:'none', cursor:'pointer', fontSize:'0.72rem', fontWeight: sidebarTab === tab.key ? '700' : '500', color: sidebarTab === tab.key ? 'var(--accent)' : 'var(--text-muted)', borderBottom: sidebarTab === tab.key ? '2px solid var(--accent)' : '2px solid transparent', transition:'all 0.2s', display:'flex', alignItems:'center', justifyContent:'center', gap:'0.35rem', fontFamily:'inherit' }}>
                      {tab.label}
                      {tab.count > 0 && <span style={{ fontSize:'0.55rem', fontWeight:'800', padding:'1px 5px', borderRadius:'10px', background: sidebarTab === tab.key ? 'var(--accent-soft)' : 'var(--bg-card-hover)', color: sidebarTab === tab.key ? 'var(--accent)' : 'var(--text-secondary)', border:`1px solid ${sidebarTab === tab.key ? 'var(--accent-border)' : 'var(--border)'}` }}>{tab.count}</span>}
                    </button>
                  ))}
                  <button onClick={() => setMobShowChat(false)}
                    style={{ padding:'0 0.85rem', background:'none', border:'none', borderLeft:'1px solid var(--border)', color:'var(--text-muted)', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                  </button>
                </div>
                {sidebarTab === 'chats' && (
                  <div style={{ padding:'0.4rem 0.75rem', display:'flex', alignItems:'center', justifyContent:'flex-end', gap:'0.4rem' }}>
                    {selectMode && selectedIds.length > 0 && (
                      <button onClick={() => setConfirmTarget('bulk')} style={{ padding:'0.28rem 0.6rem', borderRadius:'8px', border:'1px solid rgba(255,107,107,0.35)', background:'rgba(255,107,107,0.1)', color:'#ff8787', fontSize:'0.65rem', fontWeight:'700', cursor:'pointer' }}>
                        Delete {selectedIds.length}
                      </button>
                    )}
                    <button onClick={() => { setSelectMode(s => !s); setSelectedIds([]) }}
                      style={{ padding:'0.28rem 0.6rem', borderRadius:'8px', border:`1px solid ${selectMode ? 'rgba(var(--accent-rgb),0.4)' : 'var(--border)'}`, background: selectMode ? 'rgba(var(--accent-rgb),0.12)' : 'transparent', color: selectMode ? 'var(--accent)' : 'var(--text-muted)', fontSize:'0.65rem', fontWeight:'700', cursor:'pointer' }}>
              ...
              {/* List */}
              <div style={{ flex:1, overflowY:'auto', padding:'0.5rem', scrollbarWidth:'none' }} className="hide-scrollbar">
                {sidebarTab === 'requests' ? (
                  chatRequests.length === 0 ? (
                    <div style={{ textAlign:'center', padding:'3rem 1rem', color:'var(--text-ghost)' }}>
                      <div style={{ fontSize:'2rem', marginBottom:'0.5rem', opacity:0.3 }}>💬</div>
                      <p style={{ fontSize:'0.78rem', fontWeight:'500' }}>No pending requests</p>
                    </div>
                  ) : chatRequests.map(req => {
                    const isAct = activeRequest?.id === req.id
                    return (
                      <div key={req.id} onClick={() => { setActiveRequest(req); setActiveConvo(null); setMobShowChat(false) }}
                        style={{ padding:'0.75rem 1rem', borderRadius:'14px', cursor:'pointer', marginBottom:'0.25rem', position:'relative', background: isAct ? 'var(--accent-soft)' : 'transparent', border: isAct ? '1px solid var(--accent-border)' : '1px solid transparent', display:'flex', gap:'0.75rem', alignItems:'center', height:'64px', boxSizing:'border-box' }}
                        onMouseEnter={e => { if (!isAct) e.currentTarget.style.background='var(--bg-card-hover)' }}
                        onMouseLeave={e => { if (!isAct) e.currentTarget.style.background='transparent' }}>
                        <div style={{ position:'absolute', left:0, top:'50%', transform:'translateY(-50%)', width:'3px', height:'55%', borderRadius:'0 3px 3px 0', background:'linear-gradient(180deg, var(--accent), var(--accent-alt))' }} />
                        <Avatar name={`${req.sender?.firstName} ${req.sender?.lastName}`} size={38} orange={isAct} src={req.sender?.avatar || null} />
                        <div style={{ flex:1, minWidth:0 }}>
                          <div style={{ fontSize:'0.88rem', fontWeight:'700', color: isAct ? 'var(--accent)' : 'var(--text-primary)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{req.sender?.firstName} {req.sender?.lastName}</div>
                          <div style={{ fontSize:'0.7rem', color:'var(--text-muted)', marginTop:'0.15rem', display: 'flex', alignItems: 'center', gap: '0.35rem', minWidth: 0 }}>
                            <span style={{ display: 'flex', alignItems: 'center', gap: '0.2rem', color: isAct ? 'var(--accent)' : 'var(--text-muted)', fontWeight: '600', flexShrink: 0 }}>
                              {!req.itemId ? (
                                <><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>Profile</>
                              ) : (
                                <><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>Item</>
                              )}
                            </span>
                            <span style={{ color: 'var(--text-ghost)', fontSize: '0.65rem', flexShrink: 0 }}>•</span>
                  ...
                  <>
                    {newConvoMode && activeConvo?.isNew && (
                      <div style={{ padding:'0.75rem 1rem', borderRadius:'14px', background:'var(--accent-soft)', border:'1px solid var(--accent-border)', marginBottom:'0.25rem', display:'flex', gap:'0.75rem', alignItems:'center', height:'64px', boxSizing:'border-box' }}>
                    ...
                    {loadingConvos ? (
                      <div style={{ textAlign:'center', padding:'3rem 1rem' }}>
                        <div style={{ width:'28px', height:'28px', border:'2.5px solid var(--border)', borderTop:'2.5px solid var(--accent)', borderRadius:'50%', margin:'0 auto', animation:'spin 0.8s linear infinite' }} />
                      </div>
                      ...
                      conversations.map(convo => (
                        <ConversationItem key={convo.conversation_id} convo={convo}
                          isActive={activeConvo?.conversation_id === convo.conversation_id}
                          isSelected={selectedIds.includes(convo.conversation_id)} selectMode={selectMode}
                          isTyping={!(activeConvo && activeConvo.conversation_id === convo.conversation_id) && typingUserIds.has(String(convo.other_user_id))}
                          onClick={() => { setActiveConvo(convo); setActiveRequest(null); setNewConvoMode(false); setMessages([]); setMobShowChat(false) }}
        ...
        <div className="msgs-panels">
          <button ref={backRef} className="msgs-back-desktop" onClick={() => navigate(-1)} onMouseDown={onBackMouseDown} onTouchStart={onBackTouchStart}
            style={{ position:'absolute', left:'-50px', top:'12px', width:'34px', height:'34px', borderRadius:'50%', background:'var(--bg-card)', backdropFilter:'blur(12px)', WebkitBackdropFilter:'blur(12px)', border:'1.5px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'center', cursor: draggable ? 'grab' : 'pointer', flexShrink:0, color:'var(--text-muted)', transition:'all 0.15s' }}
          ...
          <div className="msgs-panel-sidebar" style={{ background: 'var(--bg-card)', backdropFilter: 'blur(20px)', border: '1px solid var(--border)', borderRadius: '20px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
              ...
              {sidebarTab === 'chats' && (
                <div style={{ padding: '0.5rem 1rem 0.5rem', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '0.5rem' }}>
                  {selectMode && selectedIds.length > 0 && (
                    <button onClick={() => setConfirmTarget('bulk')} style={{ padding: '0.3rem 0.7rem', borderRadius: '8px', border: '1px solid rgba(255,107,107,0.35)', background: 'rgba(255,107,107,0.1)', color: '#ff8787', fontSize: '0.68rem', fontWeight: '700', cursor: 'pointer', transition: 'all 0.15s' }}>
                      Delete {selectedIds.length}
                    </button>
                  )}
                  <button onClick={() => { setSelectMode(s => !s); setSelectedIds([]) }}
                    style={{ padding: '0.3rem 0.7rem', borderRadius: '8px', border: `1px solid ${selectMode ? 'rgba(var(--accent-rgb),0.4)' : 'var(--border)'}`, background: selectMode ? 'rgba(var(--accent-rgb),0.12)' : 'transparent', color: selectMode ? 'var(--accent)' : 'var(--text-muted)', fontSize: '0.68rem', fontWeight: '700', cursor: 'pointer', transition: 'all 0.15s' }}>
                    {selectMode ? 'Cancel' : 'Select'}
                  </button>
                </div>
              )}
              ...
              {sidebarTab === 'requests' ? (
                chatRequests.length === 0 ? (
                  ...
                  return (
                    <div
                      key={req.id}
                      onClick={() => { setActiveRequest(req); setActiveConvo(null) }}
                      style={{
                        padding: '0.75rem 1rem', borderRadius: '14px', cursor: 'pointer',
                        marginBottom: '0.25rem', position: 'relative',
                    ...
                    >
                      {/* accent left bar */}
                      <div style={{ position: 'absolute', left: 0, top: '50%', transform: 'translateY(-50%)', width: '3px', height: '55%', borderRadius: '0 3px 3px 0', background: 'linear-gradient(180deg, var(--accent), var(--accent-alt))' }} />
                      <Avatar name={`${req.sender?.firstName} ${req.sender?.lastName}`} size={38} orange={isActive} src={req.sender?.avatar || null} />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: '0.88rem', fontWeight: '700', color: isActive ? 'var(--accent)' : 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', letterSpacing: '-0.2px' }}>
                          {req.sender?.firstName} {req.sender?.lastName}
                        </div>
                        <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '0.15rem', display: 'flex', alignItems: 'center', gap: '0.35rem', minWidth: 0 }}>
                          <span style={{ display: 'flex', alignItems: 'center', gap: '0.2rem', color: isActive ? 'var(--accent)' : 'var(--text-muted)', fontWeight: '600', flexShrink: 0 }}>
                            {!req.itemId ? (
                              <><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>Profile</>
                            ) : (
                              <><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>Item</>
                  ...
                  {loadingConvos ? (
                    ...
                    conversations.map(convo => (
                      <ConversationItem key={convo.conversation_id} convo={convo}
                        ...
                        isSelected={selectedIds.includes(convo.conversation_id)} selectMode={selectMode}
                        isTyping={!(activeConvo && activeConvo.conversation_id === convo.conversation_id) && typingUserIds.has(String(convo.other_user_id))}
                        onClick={() => { setActiveConvo(convo); setActiveRequest(null); setNewConvoMode(false); setMessages([]); setMobShowChat(true) }}
                        onSelect={toggleSelect}
                      />
                    ))
                  )}
          ...
          <div className="msgs-panel-chat" style={{ background: 'var(--bg-card)', backdropFilter: 'blur(20px)', border: '1px solid var(--border)', borderRadius: '20px', display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden', position: 'relative' }}>
            ...
            {activeRequest && !activeConvo ? (
              <div style={{ display: 'flex', flexDirection: 'column', height: '100%', animation: 'reqSlideIn 0.22s ease' }}>

                {/* ── Header: looks like a real chat header ── */}
                <div style={{ padding: '1rem 1.5rem 0.85rem', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem' }}>
                    <button className="msgs-hamburger" onClick={() => setMobShowChat(prev => !prev)}
                      style={{ width: '32px', height: '32px', borderRadius: '9px', border: '1px solid var(--border-hover)', background: 'var(--bg-card)', color: 'var(--text-muted)', cursor: 'pointer', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.15s' }}>
                    ...
                    <Avatar name={`${activeRequest.sender?.firstName} ${activeRequest.sender?.lastName}`} size={40} src={activeRequest.sender?.avatar || null} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: '1rem', fontWeight: '700', color: 'var(--text-primary)', letterSpacing: '-0.3px' }}>
                        {activeRequest.sender?.firstName} {activeRequest.sender?.lastName}
                      </div>
                      {activeRequest.sender?.institution && (
                        <div style={{ fontSize: '0.68rem', marginTop: '0.1rem', color: 'var(--text-muted)', fontWeight: '500', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                    ...
                    {/* date badge */}
                    <div style={{ fontSize: '0.62rem', color: 'var(--text-ghost)', flexShrink: 0, padding: '0.2rem 0.6rem', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '20px' }}>
                      {activeRequest.createdAt
                        ? new Date(activeRequest.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })
                        : 'Just now'}
                    </div>
                  </div>
                </div>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem', position: 'relative' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'center' }}>
                    <div style={{
                      ...
                      <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.8" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                      Pending request
                    </div>
                  </div>
                  ...
                  {activeRequest.message ? (
                    <div style={{ display: 'flex', justifyContent: 'flex-start', animation: 'fadeUp 0.25s ease' }}>
                      <div style={{ display: 'flex', alignItems: 'flex-end', gap: '0.6rem', maxWidth: '72%' }}>
                        <Avatar
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', alignItems: 'flex-start' }}>
                          ...
                          <span style={{ fontSize: '0.6rem', color: 'var(--text-ghost)', paddingInline: '0.3rem' }}>
                            {activeRequest.createdAt
                              ? new Date(activeRequest.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
                              : ''}
                          </span>
                        </div>
                      </div>
                  ...
                  <div style={{ marginTop: 'auto', paddingTop: '1rem' }}>
                    <div style={{
                      ...
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" strokeWidth="2.2" strokeLinecap="round" style={{ flexShrink: 0, animation: 'lockPulse 2.5s ease-in-out infinite' }}>
                ...
                <div style={{ padding: '1rem 1.5rem 1.25rem', borderTop: '1px solid var(--border)', flexShrink: 0 }}>
                  <div style={{ display: 'flex', gap: '0.65rem' }}>

                    {/* Decline */}
                    <button
                      onClick={() => handleRespondRequest(activeRequest.id, 'declined')}
                      disabled={respondingId === activeRequest.id}
                      style={{
                        flex: 1, padding: '0.78rem',
                        borderRadius: 'var(--radius-md)',
                        background: 'var(--bg-card)',
                        border: '1px solid rgba(255,107,107,0.28)',
                        color: '#ff8787',
                        fontSize: '0.85rem', fontWeight: '700',
                        cursor: respondingId === activeRequest.id ? 'not-allowed' : 'pointer',
                        fontFamily: 'inherit', transition: 'all 0.2s',
                        opacity: respondingId === activeRequest.id ? 0.5 : 1,
                        display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem',
                      }}
                      onMouseEnter={e => { if (respondingId !== activeRequest.id) { e.currentTarget.style.background = 'rgba(255,107,107,0.10)'; e.currentTarget.style.borderColor = 'rgba(255,107,107,0.50)' }}}
                      onMouseLeave={e => { e.currentTarget.style.background = 'var(--bg-card)'; e.currentTarget.style.borderColor = 'rgba(255,107,107,0.28)' }}
                    >
                      {respondingId === activeRequest.id
                        ? <div style={{ width: '14px', height: '14px', border: '2px solid rgba(255,107,107,0.3)', borderTopColor: '#ff8787', borderRadius: '50%', animation: 'spin 0.6s linear infinite' }} />
                        : <>
                    ...
                    <button
                      ...
                      disabled={respondingId === activeRequest.id}
                      style={{
                        flex: 2, padding: '0.78rem',
                        borderRadius: 'var(--radius-md)',
                        background: respondingId === activeRequest.id
                        ...
                        cursor: respondingId === activeRequest.id ? 'not-allowed' : 'pointer',
                        fontFamily: 'inherit', transition: 'all 0.2s',
                        boxShadow: respondingId === activeRequest.id ? 'none' : '0 4px 18px rgba(62,207,90,0.30)',
                        opacity: respondingId === activeRequest.id ? 0.6 : 1,
                        display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem',
                      }}
                      onMouseEnter={e => { if (respondingId !== activeRequest.id) { e.currentTarget.style.filter = 'brightness(1.08)'; e.currentTarget.style.boxShadow = '0 6px 22px rgba(62,207,90,0.42)' }}}
                      onMouseLeave={e => { e.currentTarget.style.filter = ''; e.currentTarget.style.boxShadow = respondingId === activeRequest.id ? 'none' : '0 4px 18px rgba(62,207,90,0.30)' }}
                    >
                      {respondingId === activeRequest.id
                        ? <><div style={{ width: '14px', height: '14px', border: '2px solid rgba(255,255,255,0.3)', borderTopColor: 'white', borderRadius: '50%', animation: 'spin 0.6s linear infinite' }} /> Processing...</>
                        : <>
                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.8" strokeLinecap="round">
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', color: 'var(--text-ghost)' }}>
                ...
                <p style={{ fontSize: '0.85rem', fontWeight: '500' }}>
                  {sidebarTab === 'requests' ? 'Select a request to review' : 'Select a conversation'}
                </p>
              </div>
              ...
              <>
                <div style={{ padding: '1rem 1.5rem 0.85rem', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem', marginBottom: '0.85rem' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: '0.68rem', marginTop: '0.1rem', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '0.35rem',
                        color: activeConvo.isNew ? 'var(--text-ghost)' : otherUserTyping ? 'var(--accent)' : otherUserOnline ? '#4ade80' : 'var(--text-ghost)' }}>
                        {!activeConvo.isNew && (
                          <div style={{ width: '6px', height: '6px', borderRadius: '50%', flexShrink: 0,
                            background: otherUserTyping ? 'var(--accent)' : otherUserOnline ? '#4ade80' : 'var(--border-hover)',
                  ...
                  {activeConvo.item_id && <ItemCard convo={activeConvo} myId={myId} onStatusChange={handleStatusChange} />}
                </div>

                <div style={{ flex: 1, overflowY: 'auto', padding: '1.25rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                  {loadingMessages ? (
                    <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <div style={{ width: '28px', height: '28px', border: '2.5px solid var(--border)', borderTop: '2.5px solid var(--accent)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
                    </div>
                  ) : activeConvo.chat_request_status === 'pending' && activeConvo.is_request_sender && messages.length === 0 ? (
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '0.6rem', textAlign: 'center' }}>
                      <div style={{ padding: '0.6rem 0.85rem', borderRadius: '18px 18px 18px 4px', background: 'var(--bg-card-hover)', border: '1px solid var(--border-hover)', color: 'var(--text-muted)', fontSize: '0.85rem', fontStyle: 'italic', maxWidth: '60%' }}>
                        {activeConvo.last_message || 'No message attached.'}
                      </div>
                      <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', padding: '0.28rem 0.75rem', background: 'var(--accent-soft)', border: '1px solid var(--accent-border)', borderRadius: '20px', fontSize: '0.65rem', fontWeight: '700', color: 'var(--accent)' }}>
                        <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.8" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                    ...
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', textAlign: 'center' }}>
                      ...
                      {activeConvo.item_title && <p style={{ fontSize: '0.72rem', color: 'var(--text-ghost)', margin: 0 }}>about {activeConvo.item_title}</p>}
                    </div>
                  ) : messages.length === 0 ? (
                      ...
                      return (
                        <div key={msg.id || i} style={{ display: 'flex', justifyContent: isMe ? 'flex-end' : 'flex-start', animation: 'fadeUp 0.2s ease' }}>
                          <div style={{ maxWidth: '62%', display: 'flex', flexDirection: 'column', gap: '0.2rem', alignItems: isMe ? 'flex-end' : 'flex-start' }}>
                            <div style={{ padding: '0.6rem 0.95rem', borderRadius: isMe ? '18px 18px 4px 18px' : '18px 18px 18px 4px', background: isMe ? 'linear-gradient(135deg, var(--accent), var(--accent-alt))' : 'var(--bg-card-hover)', border: isMe ? 'none' : '1px solid var(--border-hover)', color: isMe ? 'white' : 'var(--text-primary)', fontSize: '0.875rem', lineHeight: '1.5', boxShadow: isMe ? '0 4px 12px rgba(var(--accent-rgb),0.22)' : 'none' }}>
                              {msg.content}
                            </div>
                ...
                <div style={{ padding: '0.85rem 1.25rem', borderTop: '1px solid var(--border)', flexShrink: 0 }}>
                  {activeConvo.chat_request_status === 'pending' && activeConvo.is_request_sender ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem', padding: '0.7rem 1rem', background: 'var(--bg-input)', border: '1px solid var(--border)', borderRadius: '12px', cursor: 'not-allowed', opacity: 0.55 }}>
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" strokeWidth="2.2" strokeLinecap="round" style={{ flexShrink: 0, animation: 'lockPulse 2.5s ease-in-out infinite' }}>
                  ...
                  <form onSubmit={handleSend} style={{ display: 'flex', gap: '0.65rem', alignItems: 'center' }}>
                    <div style={{ flex: 1 }}>
                      <input ref={msgInputRef} className="msg-input" type="text" value={newMessage}
                        ...
                        style={{ width: '100%', boxSizing: 'border-box', padding: '0.7rem 1rem', background: inputFocused ? 'var(--bg-input-focus)' : 'var(--bg-input)', border: inputFocused ? '1px solid var(--accent-border)' : '1px solid var(--border)', borderRadius: '12px', color: 'var(--text-primary)', fontSize: '0.875rem', fontFamily: 'inherit', transition: 'all 0.2s ease' }}
                      />
                    </div>
                    <button type="submit" disabled={sending || !newMessage.trim()}
                      style={{ width: '42px', height: '42px', borderRadius: '12px', flexShrink: 0, background: sending || !newMessage.trim() ? 'var(--bg-card-hover)' : 'linear-gradient(135deg, var(--accent), var(--accent-alt))', border: 'none', cursor: sending || !newMessage.trim() ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s ease', boxShadow: !sending && newMessage.trim() ? '0 4px 14px rgba(var(--accent-rgb),0.3)' : 'none' }}>
                      {sending
                        ? <div style={{ width: '14px', height: '14px', border: '2px solid var(--border-hover)', borderTopColor: 'var(--text-primary)', borderRadius: '50%', animation: 'spin 0.6s linear infinite' }} />
                        : <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={!newMessage.trim() ? 'var(--text-ghost)' : 'white'} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
                      }
                    </button>
                  </form>
                  )}
