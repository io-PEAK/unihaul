    ...
    >
      <img
        ...
        style={{
          ...
          animation: "zmScaleIn 0.28s cubic-bezier(0.175,0.885,0.32,1.275)",
          cursor: "default",
        }}
      />
      <button
    ...
    return (
      ...
      >
        <svg
          ...
          strokeWidth="1.2"
          strokeLinecap="round"
          style={{ opacity: 0.45, color: "var(--text-primary)" }}
        >
  ...
  if (n === 1)
    return (
      <>
        ...
        {zoomed && (
          <ZoomModal src={images[0]} onClose={() => setZoomed(false)} />
        )}
      </>
    );
  ...
  if (isMobile) {
    ...
    return (
      <>
          ...
          <div
            ...
            onScroll={(e) => {
              ...
              const idx = Math.round(
                el.scrollLeft / (el.offsetWidth - 48 + 12),
              );
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (track)
                      track.scrollTo({
                        left: i * (track.offsetWidth - 48 + 12),
                        behavior: "smooth",
                      });
                  }}
                  style={{
                    width: isAct ? "22px" : "7px",
                    height: isAct ? "6px" : "5px",
                    borderRadius: "999px",
                    border: "none",
                    padding: 0,
                    cursor: "pointer",
                    ...
                    boxShadow: isAct ? "0 0 8px var(--accent)" : "none",
        ...
        {zoomed && (
          <ZoomModal src={images[active]} onClose={() => setZoomed(false)} />
        )}
      </>
    );
  }
...
function ItemDetail() {
    ...
    if (!draggable) {
      ...
      try {
        const saved = JSON.parse(
          localStorage.getItem("drag_backbtn_itemdetail"),
        );
        if (saved)
  ...
  const infoGrid = [
    ...
    {
      label: "Stock",
      value: status === "sold" ? "0 remaining" : `${stockLeft}`,
    },
    ...(item.subcategory
      ? [
          {
            label: subcategoryLabel[item.category] || "Subcategory",
    ...
    ...(item.purchaseYear
      ? [
          {
            label: "Purchased",
            value: `${item.purchaseYear} · ${new Date().getFullYear() - item.purchaseYear} yr${new Date().getFullYear() - item.purchaseYear === 1 ? "" : "s"} old`,
          },
        ]
      : []),
    ...(item.expiryDate
      ? [
          {
            label: "Expires",
            value: new Date(item.expiryDate).toLocaleDateString("en-IN", {
              day: "numeric",
              month: "short",
              year: "numeric",
            }),
    ...
    ...(item.madeOn
      ? [
          {
            label: "Made On",
            value: new Date(item.madeOn).toLocaleDateString("en-IN", {
              day: "numeric",
              month: "short",
              year: "numeric",
            }),
      ...
      <div className="id-grid">
        ...
        <div className="id-detail-col">
            ...
            {infoGrid.map(({ label, value, isStatus, isSeller, sellerId }) => (
              ...
              >
                ...
                {isStatus ? (
                  ...
                  <div
                    onClick={() => sellerId && navigate(`/users/${sellerId}`)}
                    style={{
                      fontWeight: "600",
                      color: "var(--accent)",
                      fontSize: "0.9rem",
          ...
          {item.description && (
            <div
              className="glass-infobox"
              style={{ padding: "1.1rem 1.2rem", marginBottom: "1.5rem" }}
            >
              <div
                style={{
                  position: "absolute",
              ...
              />
              <div className="label-micro" style={{ marginBottom: "0.55rem" }}>
                Description
              </div>
              <p
                style={{
          ...
          {!isMyItem && user && (
            <button
              ...
              onMouseEnter={(e) => {
                if (!watchLoading) {
                  e.currentTarget.style.borderColor = watching
                    ? "rgba(255,107,107,0.6)"
                    : "var(--accent)";
                  e.currentTarget.style.color = watching
                    ? "#ff6b6b"
                    : "var(--accent)";
                }
              }}
              ...
              style={{
                width: "100%",
                padding: "0.65rem",
                background: watching ? "rgba(255,107,107,0.08)" : "transparent",
                border: `1px solid ${watching ? "rgba(255,107,107,0.35)" : "rgba(255,255,255,0.12)"}`,
                borderRadius: "var(--radius-md)",
                color: watching ? "#ff6b6b" : "var(--text-secondary)",
                fontSize: "0.82rem",
                fontWeight: "700",
          ...
          {isMyItem ? (
            ...
            cartQty === 0 ? (
              ...
              >
                ...
                >
                  ...
                  >
                    <button
                      onClick={() => handleQtyChange(cartQty - 1)}
                      disabled={cartLoading}
                      onMouseEnter={(e) =>
                      ...
                      style={{
                        ...
                        border: "none",
                        borderRight: "1px solid var(--glass-border-row)",
                        color:
                          cartQty === 1
                            ? "var(--color-danger)"
                            : "var(--text-secondary)",
                      ...
                      {stockLeft === 0 && (
                        <span
                          style={{
                            ...
                            borderRadius: "6px",
                            border: "1px solid var(--bd-pending)",
                          }}
                        >
                          max
                        </span>
                    ...
                    <button
                      onClick={() => handleQtyChange(cartQty + 1)}
                      disabled={cartLoading || stockLeft === 0}
                      onMouseEnter={(e) => {
                        if (stockLeft > 0)
                          e.currentTarget.style.background =
                            "var(--accent-soft)";
                      ...
                      style={{
                        ...
                        background: "transparent",
                        border: "none",
                        borderLeft: "1px solid var(--glass-border-row)",
                        color:
                          stockLeft === 0
                            ? "var(--text-ghost)"
                            : "var(--accent)",
                        cursor:
                          cartLoading || stockLeft === 0
                            ? "not-allowed"
                            : "pointer",
                        fontSize: "1.1rem",
                        fontWeight: "700",
            ...
            <div
              style={{
                ...
                fontSize: "0.85rem",
                background:
                  status === "pending" ? "var(--bg-pending)" : "var(--bg-sold)",
                borderRadius: "var(--radius-md)",
                border: `1px solid ${status === "pending" ? "var(--bd-pending)" : "var(--bd-sold)"}`,
              }}
            >
              {status === "pending"
