...
function ItemDetail() {
  ...
  if (error || !item) return (
    <div style={{ textAlign: 'center', padding: '6rem 2rem', color: 'rgba(255,255,255,0.25)' }}>
      ...
      <button onClick={() => navigate('/')} style={{ padding: '0.6rem 1.75rem', background: 'linear-gradient(135deg, #e87722, #f09030)', color: 'white', border: 'none', borderRadius: '12px', cursor: 'pointer', fontWeight: '700', fontSize: '0.85rem', letterSpacing: '0.5px', boxShadow: '0 4px 15px rgba(232,119,34,0.25)', transition: 'all 0.3s ease' }}>← Back to Home</button>
    </div>
  )
  ...
  async function handleAddToCart() {
    if (!user) {
      ...
      if (!guestCart.find(i => i.id === item.id)) {
        guestCart.push({ ...item, cartQty: selectedQty })
    ...
    try {
      await API.post('/cart', { itemId: item.id, quantity: selectedQty })
      navigate('/cart')
    } catch (err) {
  ...
  const infoGrid = [
    ...
    ...(item.subcategory ? [{
      label: item.category === 'Clothing' ? 'Size'
        : item.category === 'Books & Notes' ? 'Semester'
        : item.category === 'Electronics' ? 'Type'
        : 'Subcategory',
      value: item.subcategory
    }] : []),
    { label: 'Status', value: status },
    { label: 'Stock', value: `${maxQty} available` },
  ]

  return (
    <div style={{ padding: '3rem 4rem', maxWidth: '900px', margin: '0 auto' }}>

      <button onClick={() => navigate('/')} onMouseEnter={() => setBackHovered(true)} onMouseLeave={() => setBackHovered(false)}
      ...
      <div style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.08) 0%, rgba(255,255,255,0.02) 100%)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '24px', padding: '2.75rem', boxShadow: '0 8px 32px rgba(0,0,0,0.12), inset 0 1px 0 rgba(255,255,255,0.06)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1.5rem' }}>
          {infoGrid.map(info => (
            <div key={info.label} style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.05) 0%, rgba(255,255,255,0.015) 100%)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '14px', padding: '1rem 1.15rem', backdropFilter: 'blur(8px)', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ fontSize: '0.6rem', letterSpacing: '1.5px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)', marginBottom: '0.4rem', fontWeight: '700' }}>{info.label}</div>
              <div style={{ fontWeight: '600', color: 'rgba(255,255,255,0.85)', fontSize: '0.95rem', letterSpacing: '-0.2px', textTransform: 'capitalize' }}>{info.value}</div>
            </div>
          ))}
        </div>
          ...
          <div>
            ...
            {maxQty > 1 && (
              <div style={{ marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <span style={{ fontSize: '0.65rem', letterSpacing: '1.5px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)', fontWeight: '700' }}>Quantity</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px', overflow: 'hidden' }}>
                  <button
                    onClick={() => setSelectedQty(q => Math.max(1, q - 1))}
                    style={{ width: '36px', height: '36px', background: 'none', border: 'none', color: selectedQty === 1 ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.6)', cursor: selectedQty === 1 ? 'not-allowed' : 'pointer', fontSize: '1.1rem', fontWeight: '700', transition: 'all 0.2s ease', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                  >−</button>
                  <span style={{ minWidth: '36px', textAlign: 'center', color: 'white', fontWeight: '700', fontSize: '0.95rem' }}>{selectedQty}</span>
                  <button
                    onClick={() => setSelectedQty(q => Math.min(maxQty, q + 1))}
                    style={{ width: '36px', height: '36px', background: 'none', border: 'none', color: selectedQty === maxQty ? 'rgba(255,255,255,0.2)' : 'rgba(232,119,34,0.8)', cursor: selectedQty === maxQty ? 'not-allowed' : 'pointer', fontSize: '1.1rem', fontWeight: '700', transition: 'all 0.2s ease', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                  >+</button>
                </div>
                <span style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.25)', fontWeight: '500' }}>{maxQty} in stock</span>
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              ...
              >Add {selectedQty > 1 ? `${selectedQty}x` : ''} to Cart →</button>
            </div>
          </div>
        ) : (
