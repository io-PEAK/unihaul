...
function Register() {
  ...
  const fields = [
    { label: 'First Name', name: 'firstName', type: 'text', placeholder: 'Shivam' },
    { label: 'Last Name', name: 'lastName', type: 'text', placeholder: 'Nagar' },
    ...
    <div style={{ minHeight: '90vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      ...
      <div className="reg-card" style={{
        ...
        <div style={{ marginBottom: '2.25rem' }}>
          <h1 className="reg-heading" style={{ fontWeight: '900', letterSpacing: '-1.5px', lineHeight: '1.05', marginBottom: '0.6rem', color: 'var(--text-primary)' }}>
            Join the<br />
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', fontWeight: '400', letterSpacing: '0.2px' }}>
            ...
            <div key={field.name} style={{ marginBottom: '1.15rem' }}>
              <label style={{
                ...
                color: field.name === 'email' && emailError
                  ? 'var(--color-error)'
                  : focusedField === field.name ? 'var(--accent)' : 'var(--text-muted)',
              }}>{field.label}</label>
              <input
                name={field.name} type={field.type} value={form[field.name]}
                onChange={handleChange}
                ...
                style={{
                  ...
                  border: field.name === 'email' && emailError
                    ? '1px solid var(--bd-error)'
                    : focusedField === field.name ? '1px solid var(--accent-border)' : '1px solid var(--border)',
                  borderRadius: '12px', color: 'var(--text-primary)', fontSize: '0.9rem',
                  outline: 'none', transition: 'all 0.3s ease', boxSizing: 'border-box',
                }}
              />
              {field.name === 'email' && emailError && (
                <p style={{ marginTop: '0.35rem', fontSize: '0.72rem', color: 'var(--color-error)', fontWeight: '500' }}>⚠ {emailError}</p>
              )}
            </div>
          ))}

          <div style={{ height: '1px', background: 'var(--glass-divider)', margin: '1.25rem 0' }} />
