...
function SoldGroupRow({ group, isNewSale, onDelete }) {
  ...
  const sales = group.sales  // array of transactions
      ...
      <div style={{ padding: '1.25rem 1.75rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.4rem' }}>
            <h3 style={{ margin: 0, fontSize: '1.05rem', fontWeight: '700', color: 'rgba(255,255,255,0.9)', letterSpacing: '-0.3px' }}>
              {item.title}
            </h3>
            {/* Sold count badge */}
            <div style={{
              ...
              border: '1px solid rgba(255,107,107,0.15)', whiteSpace: 'nowrap',
            }}>
              Sold {sales.length}×
            </div>
            {/* New sale badge */}
            {isNewSale && (
              <div style={{
                ...
                color: '#51cf66', fontSize: '0.62rem', fontWeight: '700',
                padding: '2px 8px', borderRadius: '20px',
                animation: 'pulse-green 2.5s ease-in-out infinite',
              }}>
                <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="#51cf66" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
            ...
            <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: '0.72rem' }}>
              Listed {formatDate(item.createdAt)} · {formatTime(item.createdAt)}
            </span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginLeft: '1rem' }}>
          ...
          <button
            ...
            style={{
              padding: '0.4rem 1rem', display: 'flex', alignItems: 'center', gap: '5px',
              background: expanded ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.05)',
              color: expanded ? 'rgba(255,255,255,0.8)' : 'rgba(255,255,255,0.4)',
          ...
          <button
            ...
            style={{
              padding: '0.4rem 1rem', display: 'flex', alignItems: 'center', gap: '5px',
              background: relistHovered ? 'rgba(232,119,34,0.18)' : 'rgba(232,119,34,0.08)',
              color: relistHovered ? '#f09030' : 'rgba(232,119,34,0.7)',
              border: relistHovered ? '1px solid rgba(232,119,34,0.4)' : '1px solid rgba(232,119,34,0.2)',
              borderRadius: '10px', cursor: 'pointer', fontSize: '0.78rem', fontWeight: '600',
              transition: 'all 0.2s ease',
            }}>
          ...
          <button
            ...
            style={{
              padding: '0.4rem 1rem',
              background: deleteHovered ? 'rgba(255,107,107,0.2)' : 'rgba(255,107,107,0.08)',
              color: deleteHovered ? '#ff6b6b' : 'rgba(255,107,107,0.6)',
              border: deleteHovered ? '1px solid rgba(255,107,107,0.25)' : '1px solid rgba(255,107,107,0.1)',
              borderRadius: '10px', cursor: deleting ? 'not-allowed' : 'pointer',
      ...
      {expanded && (
        <div style={{
          ...
          <div style={{
            ...
            color: 'rgba(255,255,255,0.2)', fontWeight: '700', marginBottom: '0.75rem',
          }}>Sale History</div>
              ...
              <div key={sale.id} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <div style={{
                    ...
                    background: 'rgba(232,119,34,0.15)', border: '1px solid rgba(232,119,34,0.25)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: '0.6rem', fontWeight: '800', color: '#e87722', flexShrink: 0,
                  }}>{i + 1}</div>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '0.68rem', color: 'rgba(255,255,255,0.25)', marginTop: '1px' }}>
                      Buyer
                    </div>
                  </div>
                </div>
...
function ListingRow({ item, onDelete, onUpdate, isNewSale }) {
  const navigate = useNavigate()
  const [hovered, setHovered] = useState(false)
  ...
  async function handleSave() {
    ...
    try {
      setSaving(true)
      const res = await API.put(`/items/${item.id}`, { ...editForm, price: parsedPrice })
      onUpdate(res.data)
      setEditing(false)
    } catch (err) {
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.5rem', alignItems: 'center' }}>
              ...
              <span style={{
                ...
                background: status === 'pending' ? 'rgba(255,212,59,0.1)' : 'rgba(81,207,102,0.1)',
                padding: '2px 10px', borderRadius: '20px',
                border: status === 'pending' ? '1px solid rgba(255,212,59,0.15)' : '1px solid rgba(81,207,102,0.15)',
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <button onClick={() => setEditing(true)} onMouseEnter={() => setEditHovered(true)} onMouseLeave={() => setEditHovered(false)}
              style={{
                padding: '0.4rem 1rem',
                background: editHovered ? 'rgba(255,255,255,0.12)' : 'rgba(255,255,255,0.05)',
                color: editHovered ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.5)',
                border: editHovered ? '1px solid rgba(255,255,255,0.15)' : '1px solid rgba(255,255,255,0.06)',
                borderRadius: '10px', cursor: 'pointer', fontSize: '0.8rem', fontWeight: '600', transition: 'all 0.2s ease',
              }}>Edit</button>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
            <button onClick={() => setEditing(false)} style={{
              padding: '0.4rem 1rem', borderRadius: '10px', cursor: 'pointer',
              background: 'rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.4)',
              border: '1px solid rgba(255,255,255,0.06)', fontSize: '0.8rem', fontWeight: '600',
            }}>Cancel</button>
            <button onClick={handleSave} disabled={saving} style={{
              padding: '0.4rem 1.25rem', borderRadius: '10px', cursor: saving ? 'not-allowed' : 'pointer',
              background: saving ? 'rgba(255,255,255,0.06)' : 'linear-gradient(135deg, #e87722, #f09030)',
              color: saving ? 'rgba(255,255,255,0.3)' : 'white',
              border: 'none', fontSize: '0.8rem', fontWeight: '700',
              boxShadow: saving ? 'none' : '0 4px 15px rgba(232,119,34,0.3)',
...
function Dashboard() {
  ...
  // My sold transactions keyed by item_id
  const myId = user.id
  const soldTxns = transactions.filter(t => t.seller_id === myId)

  // Group sold items: one entry per unique item_id with all its sales
  const soldGroups = (() => {
    const map = new Map()
    soldTxns.forEach(t => {
  ...
  return (
    <div onClick={() => navigate(-1)} style={{ minHeight: 'calc(100vh - 70px)', cursor: 'default' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '2rem', alignItems: 'center' }}>
          {[
            ...
            <div key={stat.label} style={{
              ...
              borderRadius: '14px', padding: '0.85rem 1.25rem', minWidth: '72px',
          ...
          <button
            ...
            style={{
              ...
              background: postBtnHovered ? 'linear-gradient(135deg, #f09030, #e87722)' : 'linear-gradient(135deg, #e87722, #f09030)',
              color: 'white', border: 'none', borderRadius: '14px',
              fontSize: '0.8rem', fontWeight: '700', cursor: 'pointer',
              letterSpacing: '1px', textTransform: 'uppercase',
              transition: 'all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)',
              transform: postBtnHovered ? 'translateY(-3px)' : 'translateY(0)',
              boxShadow: postBtnHovered ? '0 15px 35px rgba(232,119,34,0.35)' : '0 4px 15px rgba(232,119,34,0.2)',
              whiteSpace: 'nowrap',
            }}
          ...
          {FILTERS.map(f => {
            ...
            return (
              <button key={f.key} onClick={() => setActiveFilter(f.key)} style={{
                ...
                background: isActive ? 'linear-gradient(135deg, #e87722, #f09030)' : 'rgba(255,255,255,0.06)',
                color: isActive ? 'white' : 'rgba(255,255,255,0.5)',
                border: isActive ? 'none' : '1px solid rgba(255,255,255,0.08)',
                borderRadius: '12px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: '700',
                transition: 'all 0.25s ease',
                boxShadow: isActive ? '0 4px 15px rgba(232,119,34,0.3)' : 'none',
              }}>{f.label}</button>
            )
          })}
        </div>
        ...
        {!loading && !error && (activeFilter === 'active' || activeFilter === 'pending') && (() => {
          ...
          if (list.length === 0) return (
            <div style={{ textAlign: 'center', padding: '4rem 2rem', background: 'linear-gradient(135deg, rgba(255,255,255,0.04) 0%, rgba(255,255,255,0.01) 100%)', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <p style={{ color: 'rgba(255,255,255,0.25)', fontWeight: '500' }}>No {activeFilter} listings yet.</p>
            </div>
          )
          return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {list.map(item => (
                <ListingRow key={item.id} item={item} onDelete={handleDelete} onUpdate={handleUpdate} isNewSale={newSaleItemIds.has(item.id)} />
              ))}
            </div>
          )
        })()}

        {/* All — non-sold as ListingRow + sold as SoldGroupRow */}
        {!loading && !error && activeFilter === 'all' && (() => {
          const hasAnything = allNonSoldItems.length > 0 || soldGroups.length > 0
          if (!hasAnything) return (
            <div style={{ textAlign: 'center', padding: '4rem 2rem', background: 'linear-gradient(135deg, rgba(255,255,255,0.04) 0%, rgba(255,255,255,0.01) 100%)', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.06)' }}>
              <div style={{ fontSize: '2.5rem', marginBottom: '1rem', opacity: 0.4 }}>📭</div>
              <p style={{ color: 'rgba(255,255,255,0.25)', fontWeight: '500' }}>No listings yet.</p>
            </div>
          )
          return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              ...
              {soldGroups.map(group => (
                <SoldGroupRow key={group.item.id} group={group} isNewSale={newSaleItemIds.has(group.item.id)} onDelete={handleDelete} />
              ))}
            </div>
          )
        })()}

        {/* Sold — grouped rows with history */}
        {!loading && !error && activeFilter === 'sold' && (() => {
          if (soldGroups.length === 0) return (
            <div style={{ textAlign: 'center', padding: '4rem 2rem', background: 'linear-gradient(135deg, rgba(255,255,255,0.04) 0%, rgba(255,255,255,0.01) 100%)', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.06)' }}>
              <div style={{ fontSize: '2.5rem', marginBottom: '1rem', opacity: 0.4 }}>📭</div>
              <p style={{ color: 'rgba(255,255,255,0.25)', fontWeight: '500' }}>No sold items yet.</p>
            </div>
          )
          return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {soldGroups.map(group => (
                <SoldGroupRow
                  key={group.item.id}
                  group={group}
                  isNewSale={newSaleItemIds.has(group.item.id)}
