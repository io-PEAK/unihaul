...
const specFieldsMap = {
  ...
  "Sports & Fitness": [
    ...
    },
    {
      key: "brand",
      label: "Brand",
      placeholder: "e.g. Nike, Adidas, SG",
      span: 1,
    },
  ...
  Stationery: [
    {
      key: "type",
      label: "Type",
      placeholder: "e.g. Notebook, Pen set",
      span: 1,
    },
    {
      key: "brand",
      label: "Brand",
      placeholder: "e.g. Classmate, Natraj",
      span: 1,
    },
  ],
  Appliances: [
  ...
  "Games & Hobbies": [
    ...
    },
    {
      key: "type",
      label: "Type",
      placeholder: "e.g. Strategy, Action",
      span: 1,
    },
    {
      key: "brand",
      label: "Brand",
      placeholder: "e.g. Sony, Nintendo, EA",
      span: 2,
    },
  ],
  Services: [
...
const specValidators = {
  ...
  Furniture: {
    ...
    dimensions: (v) =>
      badStart(v) || (pureNum(v) ? "Add a unit — e.g. 120×60 cm" : null),
  },
  "Sports & Fitness": {
    sport: (v) =>
      badStart(v) ||
        ...
        style={{
          ...
          background: isFocused
            ...
            : hasError
              ...
              : "var(--bg-input)",
          border: isFocused
            ? `1px solid ${hasError ? "rgba(255,107,107,0.6)" : "var(--accent-border)"}`
            : hasError
              ? "1px solid rgba(255,107,107,0.35)"
              : value
                ? "1px solid var(--accent-border)"
                : "1px solid var(--glass-border)",
          borderRadius: showDrop || hasError ? "10px 10px 0 0" : "10px",
...
function ImageUploadZone({
  ...
  const previewSrc =
    previewIdx !== null
      ? images[previewIdx]?.url || images[previewIdx]?.preview
      : null;
      ...
      <div style={{ marginBottom: "1.15rem" }}>
        ...
        >
          ...
          {images.length >= 2 && (
            ...
            >
              ({images.length}/{MAX}) · drag to reorder
            </span>
          )}
        </div>
        ...
        >
          {images.map((img, i) => (
            <div
              key={img._uid || img.url || img.preview}
              draggable
          ...
          {Array.from({ length: Math.max(0, MAX - images.length - 1) }).map(
            (_, i) => (
              <div
                key={`empty-${i}`}
                style={{
...
function PostItem() {
    ...
    if (failed.length > 0) {
      ...
      const messages = failed.map((f) => {
        ...
        if (
          ...
          errMsg.includes("large") ||
          errMsg.includes("size")
        )
          return `${short} is over 5MB`;
        if (
          f.status === 400 ||
          errMsg.includes("type") ||
          errMsg.includes("format")
        )
          return `${short} is an invalid file type`;
  ...
  function removeImage(idx) {
    ...
    if (img?.publicId) {
      ...
      uploadedPublicIds.current = uploadedPublicIds.current.filter(
        (id) => id !== img.publicId,
      );
    }
  ...
  async function handleSubmit(e) {
    ...
    if (
      ...
      setError("Purchase year is required for this category.");
      return;
    }
    if (/^[^a-zA-Z0-9]/.test(form.title.trim())) {
      setError("Title can't start with a special character.");
      return;
    }
    if (/^\d/.test(form.title.trim())) {
      setError("Title can't start with a number.");
      return;
    }
    if (form.title.trim().length < 10) {
      setError("Title too short (min 10 chars).");
      return;
    }
    ...
    if (uploadedImages.length < 2) {
      setError(
        "Please add at least 2 photos — buyers trust listings more with multiple images.",
      );
      return;
    }
  ...
  const inputStyle = (name) => ({
    ...
    padding: "0.7rem 1rem",
    background:
      focusedField === name ? "var(--bg-input-focus)" : "var(--bg-input)",
    border:
      focusedField === name
        ? "1px solid var(--accent-border)"
        : "1px solid var(--glass-border)",
        ...
        <div ref={scrollRef} className="pi-scroll pi-panel">
          ...
          <div style={{ marginBottom: "2rem" }}>
            ...
            <p style={{ color: "var(--text-muted)", fontSize: "0.85rem" }}>
              {prefill
                ? "Edit the details and repost your listing"
                : "Fill in the details to post your listing"}
            </p>
          </div>
          ...
          {success && (
            <div
              style={{
                ...
                background:
                  "linear-gradient(135deg, rgba(81,207,102,0.15) 0%, rgba(64,192,87,0.08) 100%)",
            ...
            >
              <div
                style={{
                  ...
                  height: "1px",
                  background:
                    "linear-gradient(90deg, transparent, rgba(81,207,102,0.6), transparent)",
                }}
              />
              <div
                style={{
              ...
              >
                <div
                  style={{
                    ...
                    borderRadius: "50%",
                    background:
                      "linear-gradient(135deg, rgba(81,207,102,0.35), rgba(64,192,87,0.2))",
                    border: "1.5px solid rgba(81,207,102,0.5)",
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    flexShrink: 0,
                    boxShadow: "0 0 12px rgba(81,207,102,0.3)",
                  }}
                >
                  <svg
                    width="14"
          ...
          <form onSubmit={handleSubmit} style={{ overflow: "visible" }}>
            ...
            <div className="pi-form-row">
              ...
              <div>
                ...
                <CustomSelect
                  ...
                  options={["Like New", "Good", "Fair", "Poor"].map((c) => ({
                    value: c,
                    label: c,
                  }))}
            ...
            {specFields.length > 0 && (
              <div
                style={{
                  ...
                  padding: "1.15rem",
                  animation:
                    "specsIn 0.3s cubic-bezier(0.175,0.885,0.32,1.275)",
                  overflow: "visible",
                  position: "relative",
                  zIndex: 10,
                }}
            ...
            <button
              ...
              style={{
                ...
                padding: "0.8rem",
                background: success
                  ? "linear-gradient(135deg, rgba(81,207,102,0.25), rgba(64,192,87,0.15))"
                  : loading
                ...
                color: success
                  ...
                  : loading
                    ...
                    : "white",
                border: success ? "1px solid rgba(81,207,102,0.4)" : "none",
                borderRadius: "12px",
                fontSize: "0.85rem",
                fontWeight: "700",
                ...
                boxShadow: success
                  ? "0 4px 20px rgba(81,207,102,0.2)"
                  : btnHovered && !loading
