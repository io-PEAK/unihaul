  ...
  useEffect(() => {
    function onOut(e) {
      if (
        ...
        !e.target.closest("[data-portal-dropdown]")
      ) {
              ...
              style={{
                ...
                color:
                  hoveredIdx === i
                    ...
                    : "var(--text-secondary)",
                background:
                  hoveredIdx === i ? "var(--accent-soft)" : "transparent",
              ...
              {matchIdx >= 0 ? (
                <span>
                  ...
                  <span style={{ color: "var(--accent)", fontWeight: "700" }}>
                    {s.slice(matchIdx, matchIdx + value.length)}
                  </span>
          ...
          style={{
            ...
            padding:
              suggestions.length > 0
                ? "0.52rem 1.8rem 0.52rem 0.85rem"
                : "0.52rem 0.85rem",
              ...
              style={{
                ...
                background: isActive
                  ...
                  : hIdx === i
                    ...
                    : "transparent",
                borderBottom:
                  i < options.length ? "1px solid var(--border)" : "none",
    ...
    >
      <span
        style={{
          fontSize: "0.73rem",
          fontWeight: "600",
          color: h ? "var(--accent-alt)" : "var(--accent)",
        }}
      >
        {label}
      </span>
      <span
        style={{
          fontSize: "0.8rem",
          color: h ? "var(--accent-alt)" : "rgba(var(--accent-rgb),0.5)",
          lineHeight: 1,
          marginTop: "-1px",
        }}
      >
...
// ─── Status badge ─────────────────────────────────────────────────────────────
function StatusBadge({ status, small = false, isWatching = false }) {
    ...
    <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
      {isWatching && (
        <div
          style={{
            ...
            borderRadius: "50%",
            background: "linear-gradient(135deg, rgba(255,77,77,0.18) 0%, rgba(255,120,120,0.10) 100%)",
            backdropFilter: "blur(10px)",
            WebkitBackdropFilter: "blur(10px)",
            border: "1px solid rgba(255,99,99,0.30)",
            boxShadow: "0 2px 10px rgba(255,77,77,0.30), inset 0 1px 0 rgba(255,255,255,0.10)",
      ...
      <span
        style={{
          ...
          border:
            s === "sold"
              ? "1px solid var(--bd-sold)"
              : s === "pending"
                ? "1px solid var(--bd-pending)"
                : "1px solid var(--bd-available)",
        }}
      >
        {s}
      </span>
          ...
          >
            ...
            <div
              style={{
                ...
                inset: 0,
                background:
                  "linear-gradient(135deg, rgba(0,0,0,0.08) 0%, transparent 60%)",
                pointerEvents: "none",
              }}
            />
          </div>
    ...
    <>
      ...
      >
        ...
        <div style={{ padding: "0.85rem 1.25rem 1.25rem" }}>
          {/* Category Label moved here to match Grid II style */}
          <div
            style={{
              display: "flex",
              alignItems: "center",
...
function Home() {
  ...
  useEffect(() => {
    ...
    window.__homeGridBridge = {
      set: (val) => setGridSizeState(Math.max(2, val)),
    };
  ...
  useEffect(() => {
    ...
    fetchWatched();
    window.addEventListener("watching-updated", fetchWatched);
    ...
    const obs = new IntersectionObserver(
      ([entry]) => {
        window.dispatchEvent(
          new CustomEvent("home-searchbar-visibility", {
  ...
  useEffect(() => {
    window.dispatchEvent(
      new CustomEvent("navbar-filter-state", {
  ...
  const chips = [
    filters.category && {
      ...
      remove: () =>
        setFilters((f) => ({ ...f, category: "", subcategory: "", specs: {} })),
    },
    filters.subcategory && {
      id: "sub",
      label: filters.subcategory,
      remove: () => setFilters((f) => ({ ...f, subcategory: "" })),
    },
    filters.sortPrice && {
      id: "sort",
      label: sortOptions.find((s) => s.value === filters.sortPrice)?.label,
      remove: () => setFilters((f) => ({ ...f, sortPrice: "" })),
    },
    ...Object.entries(filters.specs || {})
      .filter(([, v]) => v)
      .map(([k, v]) => ({
        id: `spec-${k}`,
        label: `${k}: ${v}`,
        remove: () =>
          setFilters((f) => ({ ...f, specs: { ...f.specs, [k]: "" } })),
      })),
  ].filter(Boolean);
  ...
  return (
    <div
      className="home-page"
      style={{ padding: "3rem 4rem", maxWidth: "1200px", margin: "0 auto" }}
    >
      <style>{`
        @keyframes heartPulse { 0%,100% { transform:scale(1); box-shadow:0 2px 10px rgba(255,77,77,0.30),inset 0 1px 0 rgba(255,255,255,0.10); } 50% { transform:scale(1.15); box-shadow:0 4px 18px rgba(255,77,77,0.55),inset 0 1px 0 rgba(255,255,255,0.14); } }
        ...
        >
          ...
          <input
            ...
            onChange={(e) => {
              ...
              window.dispatchEvent(
                new CustomEvent("home-search-changed", {
                  detail: { value: val },
                }),
              );
            }}
          ...
          <button
            ...
            style={{
              ...
              flexShrink: 0,
              animation:
                activeCount > 0 ? "btnGlow 2.8s ease-in-out infinite" : "none",
            }}
          >
            <FilterIcon
              color={
              ...
              strokeWidth={activeCount > 0 ? 2.5 : 2.2}
            />
            <span
                ...
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  {sortOptions.map((opt) => (
                    <button
                      key={opt.value}
                      onClick={() =>
                        setFilters((f) => ({ ...f, sortPrice: opt.value }))
                      }
                      style={{
                        flex: 1,
                      ...
                      <div key={f.key}>
                        ...
                        <FilterSpecInput
                          ...
                          onChange={(val) =>
                            setFilters((d) => ({
                              ...d,
                              specs: { ...d.specs, [f.key]: val },
                            }))
                          }
                          suggestions={
                            (specSuggestionsMap[filters.category] || {})[
                              f.key
                            ] || []
                          }
                          openKey={openSpecKey}
