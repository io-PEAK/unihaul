  ...
  const startDrag = useCallback(
    (clientX, clientY) => {
      ...
      const move = (cx, cy) => {
        ...
        const newX = Math.max(
          8,
          Math.min(window.innerWidth - w - 8, startElem.current.x + dx),
        );
        const newY = Math.max(
          8,
          Math.min(window.innerHeight - h - 8, startElem.current.y + dy),
        );
      ...
      const up = () => {
        ...
        if (hasDragged.current) {
          ...
          const finalPos = {
            top: `${curPos.current.y}px`,
            left: `${curPos.current.x}px`,
            bottom: "auto",
            right: "auto",
          };
          setPos(finalPos);
