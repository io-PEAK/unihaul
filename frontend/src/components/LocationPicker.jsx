...
function buildPillLabel(location) {
  ...
  if (parts.length === 1)
    return parts[0].length > 22 ? parts[0].slice(0, 20) + "…" : parts[0];
...
export default function LocationPicker({ location, setLocation }) {
  ...
  const userInstitution = (() => {
    try {
      return JSON.parse(localStorage.getItem("user"))?.institution || null;
    } catch {
      return null;
    }
  })();
  ...
  const updatePos = useCallback(() => {
    if (btnRef.current) {
      ...
      setDropPos({
        top: r.bottom + 6,
        left: Math.max(8, r.left + r.width / 2 - 150),
      });
    }
  }, []);
  ...
  function toggleAllIndia() {
    setLocation({ mode: "all", institutions: [], cities: {}, statesOnly: {} });
  }
  ...
  function toggleInstitution(name) {
    setLocation((prev) => {
      ...
      const next = has
        ? prev.institutions.filter((i) => i !== name)
        : [...prev.institutions, name];
      const isEmpty =
        next.length === 0 &&
        Object.keys(prev.cities).length === 0 &&
        Object.keys(prev.statesOnly).length === 0;
      return isEmpty
        ? { mode: "all", institutions: [], cities: {}, statesOnly: {} }
        : { ...prev, mode: "custom", institutions: next };
    });
  }
  ...
  function toggleCity(state, city) {
    setLocation((prev) => {
      ...
      const isEmpty =
        prev.institutions.length === 0 &&
        Object.keys(newCities).length === 0 &&
        Object.keys(newStatesOnly).length === 0;
      return isEmpty
        ? { mode: "all", institutions: [], cities: {}, statesOnly: {} }
        : {
            ...prev,
      ...
      `}</style>
        ...
        >
          ...
          <div
            className="loc-row"
            onClick={toggleAllIndia}
            style={rowStyle(isAllIndia)}
          >
            <Checkbox checked={isAllIndia} />
            <span
              style={{
                fontSize: "0.82rem",
                fontWeight: isAllIndia ? "700" : "500",
                color: isAllIndia ? "var(--accent)" : "var(--text-primary)",
              }}
            >
              All India
            </span>
            ...
            <div>
              {userInstitution && (
                <div style={{ marginBottom: "0.5rem" }}>
                  ...
                  <div
                    className="loc-row"
                    onClick={() => toggleInstitution(userInstitution)}
                    style={rowStyle(
                      location.institutions.includes(userInstitution),
                    )}
                  >
                    <Checkbox
                      checked={location.institutions.includes(userInstitution)}
                    />
                    <span
                      style={{
                        fontSize: "0.8rem",
                        fontWeight: location.institutions.includes(
                          userInstitution,
                        )
                          ? "700"
                          : "500",
                        color: location.institutions.includes(userInstitution)
                          ? "var(--accent)"
                          : "var(--text-primary)",
                        lineHeight: 1.3,
                      }}
              ...
              {!instLoading && instQuery && instResults.length === 0 && (
                <div
                  style={{
                    fontSize: "0.75rem",
                    color: "var(--text-muted)",
            ...
            <div>
              ...
              {filteredStates.map((state) => {
                ...
                const visibleCities = areaQuery.trim()
                  ? loadedCities.filter((c) =>
                      c.toLowerCase().includes(areaQuery.toLowerCase()),
                    )
                  : loadedCities;
                return (
                  <div key={state} style={{ marginBottom: "2px" }}>
                    ...
                    >
                      <div
                        className="loc-row"
                        onClick={() => toggleStateOnly(state)}
                        style={{ ...rowStyle(stateChecked), flex: 1 }}
                      >
                        <Checkbox checked={stateChecked} />
                        <span
                          style={{
                            fontSize: "0.8rem",
                            fontWeight: stateChecked ? "700" : "500",
                            color: stateChecked
                              ? "var(--accent)"
                              : "var(--text-primary)",
                            flex: 1,
                          ...
                          visibleCities.map((city) => {
                            ...
                            return (
                              <div
                                ...
                                style={rowStyle(cityChecked)}
                              >
                                <Checkbox checked={cityChecked} />
                                <span
                                  style={{
                                    fontSize: "0.75rem",
                                    fontWeight: cityChecked ? "700" : "400",
                                    color: cityChecked
                                      ? "var(--accent)"
                                      : "var(--text-secondary)",
                                  }}
    ...
    <>
      ...
      >
        <LocationIcon
          size={14}
          color={
            open || isActive || hovered ? "var(--accent)" : "var(--text-muted)"
          }
        />
        <span
          style={{
